...
#filter by analyte and the LOD based thresholds
```{r}
#filter data by thresholds
bcpp_filtered <- filter_analyte(data=demog_exp, 
                                analyte = "bis(2-ethylhexyl) phosphate",
                                lod_minus = -6.481038227, 
                                lod_plus = -4.806674153, 
                                lod_value = -5.6439,
                                label_type = "quart_sd")
```
...
df_summ <- bcpp_filtered |> filter(analyte == "bis(2-ethylhexyl) phosphate")
generate_tables(df=df_summ, 
...
```{r}
bcpp_filtered$result_res<-  log2(bcpp_filtered$result_res)
analyte_summary(bcpp_filtered, analyte, category, result_res)
```
